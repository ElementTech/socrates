...
@Component({
  ...
  styleUrls: ['./block-stats.component.css']
})
export class BlockStatsComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.block_id = this.actRoute.snapshot.paramMap.get('id')
 ...
 init(){
  this.apiService.getBlockStatsByBlockID(this.block_id).subscribe(data=>
    {
      this.single = [{"name": "Success","value": data.success},{"name": "Failure","value": data.fail}]
      this.avg = data.avg
      let test = data.runs.reduce((a,v) => ((a[v.instance] = a[v.instance] || []).push(v), a), {});
      this.multi = Object.entries(test).map((e) => ( { "name":[e[0]][0],"series": e[1] } ))
