...
export class FlowListComponent implements OnInit {
  ...
  fetchLastBuild(thisFlow) {
    ...
    for (let index = 0; index < thisFlow.length; index++) {
      ...
      this.apiService.getFlowInstanceByFlowID(element._id).subscribe(data => {
        thisFlow[index].steps = thisFlow[index].steps.length
    ...
    this.confirmationService.confirm({
        ...
        accept: () => {
            ...
            this.selectedFlows.forEach(flowToDelete=>{
              runs.push(this.apiService.deleteFlow(flowToDelete._id.toString()).toPromise());
            });
  ...
  ngOnInit() {
      this.apiService.getFlows().subscribe(flows => {
          this.flows = flows;
